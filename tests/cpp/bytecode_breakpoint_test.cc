...
// TODO(b/198356867): Opensource this test by fixing the import issue.
// The import statement causes issues in the Bazel build since it can't find
// the library and there doesn't seem to be a direct way to change the Python
// interperter being used in the C++ code.
TEST_F(BytecodeBreakpointTest, AsyncAwait) {
  // Do 1 iteration without the breakpoint, 1 iteration with, then 1 without.
  TestMethod test_method = DefineMethod({
    "import asyncio",
    "def test():",
    "  async def inner():",
    "    for i in range(3):",
    "      if i == 1:",
    "        dummy = None  # BPTAG: SET",
    "      if i == 2:",
    "        dummy = None  # BPTAG: REMOVE",
    "      await asyncio.sleep(0)",
    "      await asyncio.sleep(0)  # BPTAG: AWAIT1",
    "      await asyncio.sleep(0)  # BPTAG: AWAIT2",
    "      dummy = None  # BPTAG: AWAIT3",
    "  loop = asyncio.get_event_loop()",
    "  loop.run_until_complete(inner())",
    "  loop.close()",
  });

  TestMethod inner_method = GetInnerMethod(test_method, "inner");
  ...
  SetBreakpoint(
      inner_method,
      "SET",
      [this, &counter, &inner_method, &cookie1, &cookie2, &cookie3] () {
        cookie1 = SetCountingBreakpoint(inner_method, "AWAIT1", &counter);
        cookie2 = SetCountingBreakpoint(inner_method, "AWAIT2", &counter);
        cookie3 = SetCountingBreakpoint(inner_method, "AWAIT3", &counter);
      });

  SetBreakpoint(
      inner_method,
      "REMOVE",
      [this, &cookie1, &cookie2, &cookie3] () {
        emulator_.ClearBreakpoint(cookie1);
        emulator_.ClearBreakpoint(cookie2);
        emulator_.ClearBreakpoint(cookie3);
      });
